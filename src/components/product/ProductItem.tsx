    ...
    <div className="mb-3 mx-[12.5px] max-[625px]:mx-[5px]">
      <div className="mb-2  w-[213px] h-[213px] max-[865px]:w-[170px] max-[865px]:h-[170px] rounded-lg  object-cover overflow-hidden relative">
        <div className="w-full h-full rounded-lg bg-black bg-opacity-70 absolute opacity-0 hover:opacity-100 transition-all duration-[0.3s] ease-out  z-10">
          <div
            className="w-[30px] h-[30px] absolute right-2 top-2 rounded-full bg-white flex items-center justify-center"
            onClick={(e) => handleLike(e, item.id)}
          >
            {likes ? (
              <FaHeart className="text-[red]" />
            ) : (
              <FaRegHeart className="text-[#D6D6D6]" />
            )}
          </div>
        ...
        <Image
          ...
          fill
          sizes="1"
          className="w-[228px] h-[228px] object-cover mb-2"
          alt="물품이미지"
          priority
        />
      </div>
      <div>
        <p className="text-white text-[20px] leading-[24px] mb-[6px] max-[865px]:text-[16px] max-[625px]:text-[14px] max-[865px]:leading-none">
          {item.product_name}
          <span>&nbsp;{item.product_grade}</span>
        </p>
        <p className="text-[#868686] text-[16px] leading-[24px] mb-[6px] max-[865px]:text-[16px] max-[625px]:text-[14px] max-[865px]:leading-none">
          {item.category}
        </p>
        <div className="flex items-center max-[625px]:block mb-[6px]">
          <p className="text-white text-nowrap">시작가&nbsp;</p>
          <p className="text-white text-nowrap text-[20px] leading-[24px] max-[865px]:text-[16px] max-[625px]:text-[14px] max-[865px]:leading-none">
            {item.product_bid_price.toLocaleString()}원
          </p>
        </div>
        <div className="flex items-center max-[625px]:block">
          <p className="text-[#D1B383] text-nowrap">현재가&nbsp;</p>
          <p className="text-[#D1B383] text-nowrap text-[20px] leading-[24px] max-[865px]:text-[16px] max-[625px]:text-[14px] max-[865px]:leading-none">
